 ...
 * - Save and manage reusable requests with folder organization
 * - Introspection support
 * - Request preview, curl/postman export
...
/** Convert headers object to key-value array */
...
export function GraphQLClient() {
    ...
    const handlePreview = async () => {
        try {
            const res = await apiPost('/api/graphqlclient/preview', getRequestBody());
            setPreviewData(res);
    ...
    const handleExport = async (format) => {
        try {
            const res = await apiPost('/api/graphqlclient/export/' + format, getRequestBody());
    ...
    const handleSaveRequest = async () => {
        ...
        try {
            await apiPost('/api/graphqlclient/requests/save', {
                ...
                headers_json: kvToJSON(headerKV),
                environment_id: selectedEnvId,
                auth_provider_id: selectedAuthId,
                operation_name: operationName,
            });
    ...
    const loadSavedRequest = async (reqId) => {
        try {
            ...
            setSaveFolderName(res.folder_name || '');
    ...
    const handleRenameFolder = async () => {
        ...
        try {
            await apiPost('/api/graphqlclient/folders/rename', { old_name: renameFolderTarget, new_name: renameFolderValue.trim() });
    ...
    const handleCreateFolder = () => {
        ...
        setSaveFolderName(newFolderName.trim());
        setShowNewFolder(false);
        setNewFolderName('');
        toast.info('Folder "' + newFolderName.trim() + '" selected. Save a request to create it.');
    };

    // Header key-value management
    const updateHeaderKV = (idx, field, val) => {
        setHeaderKV(prev => prev.map((h, i) => i === idx ? { ...h, [field]: val } : h));
    };
    ...
    const toggleFolder = (fname) => setOpenFolders(prev => ({ ...prev, [fname]: !prev[fname] }));

    const selectedEnv = environments.find(e => e.id === selectedEnvId);

    // Build folder tree
    const folderMap = {};
    const unfolderedRequests = [];
    savedRequests.forEach(r => {
        if (r.folder_name) {
            if (!folderMap[r.folder_name]) folderMap[r.folder_name] = [];
            folderMap[r.folder_name].push(r);
        } else {
            unfolderedRequests.push(r);
        }
    });
    ...
    const renderRequestItem = (r) => {
        ...
        return html`
            <div key=${r.id} style="padding: 4px 12px 4px 24px; cursor: pointer; display: flex; align-items: center; gap: 6px; font-size: var(--font-size-xs);"
                onClick=${() => loadSavedRequest(r.id)}
                onMouseOver=${(e) => { e.currentTarget.style.background = 'var(--bg-tertiary)'; e.currentTarget.querySelector('.del-btn').style.visibility = 'visible'; }}
                onMouseOut=${(e) => { e.currentTarget.style.background = 'transparent'; e.currentTarget.querySelector('.del-btn').style.visibility = 'hidden'; }}>
                <span style=${{ color: isMutation ? 'var(--color-warning)' : 'var(--color-info)', fontWeight: 600, fontSize: '10px', minWidth: '14px' }}>${isMutation ? 'M' : 'Q'}</span>
                <span style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1;" title=${r.name}>${r.name}</span>
                <button class="btn btn-ghost del-btn" style="padding: 2px; min-width: 0; opacity: 0.5; visibility: hidden;"
        ...
        <div style="display: flex; flex-direction: column; height: calc(100vh - 80px);">
            ...
            ${selectedEnv && html`
                <div style="flex-shrink: 0; padding: 6px 12px; background: var(--bg-tertiary); border-radius: var(--radius-md); margin-bottom: 8px; display: flex; gap: 16px; align-items: center; font-size: var(--font-size-xs);">
                    ...
                    ${!selectedEnv.verify_ssl && html`<span style="color: var(--color-warning);">SSL verification disabled</span>`}
                </div>
            `}
            ...
            <div style="flex: 1; display: flex; gap: 0; min-height: 0; overflow: hidden;">
                <!-- Collection Sidebar -->
                ${collectionOpen && html`
                    <div style="width: 240px; flex-shrink: 0; border-right: 1px solid var(--border-primary); display: flex; flex-direction: column; overflow: hidden;">
                        ...
                        ${showNewFolder && html`
                            <div style="padding: 6px 12px; border-bottom: 1px solid var(--border-weak); display: flex; gap: 4px;">
                                <input class="form-input" style="font-size: 11px; padding: 4px 6px;" value=${newFolderName}
                                ...
                                <button class="btn btn-ghost" style="padding: 2px; min-width: 0;" onClick=${() => setShowNewFolder(false)}><${Icon} name="x" size=${12} /></button>
                            </div>
                        `}
                        <div style="flex: 1; overflow-y: auto; padding: 4px 0;">
                            ${folderNames.map(fname => html`
                                <div key=${fname}>
                                    <div style="padding: 4px 12px; display: flex; align-items: center; gap: 4px; cursor: pointer; font-size: var(--font-size-xs);"
                                        onClick=${() => toggleFolder(fname)}
                                        onMouseOver=${(e) => e.currentTarget.style.background = 'var(--bg-tertiary)'}
                                        onMouseOut=${(e) => e.currentTarget.style.background = 'transparent'}>
                                        <${Icon} name=${openFolders[fname] ? 'chevron-down' : 'chevron-right'} size=${10} />
                                        <${Icon} name="folder" size=${12} style=${{ color: 'var(--color-warning)' }} />
                                        <span style="font-weight: 600; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${fname}</span>
                                        <span class="text-muted" style="font-size: 10px;">${folderMap[fname].length}</span>
                                        <button class="btn btn-ghost" style="padding: 1px; min-width: 0; opacity: 0.4;"
                                            onClick=${(e) => { e.stopPropagation(); setRenameFolderTarget(fname); setRenameFolderValue(fname); }}
                                            title="Rename"><${Icon} name="edit" size=${10} /></button>
                                        <button class="btn btn-ghost" style="padding: 1px; min-width: 0; opacity: 0.4;"
                                            onClick=${(e) => { e.stopPropagation(); setDeleteFolderTarget(fname); }}
                                            title="Delete"><${Icon} name="trash" size=${10} /></button>
                                    </div>
                                    ${openFolders[fname] && folderMap[fname].map(r => renderRequestItem(r))}
                                </div>
                            `)}
                            ${unfolderedRequests.length > 0 && html`
                                ${folderNames.length > 0 && html`
                                ...
                                `}
                                ${unfolderedRequests.map(r => renderRequestItem(r))}
                            `}
                            ...
                            <div style="background: var(--bg-primary); border: 1px solid var(--border-weak); border-radius: var(--radius-md); padding: 8px; max-height: 140px; overflow-y: auto;">
                                ${headerKV.map((h, i) => html`
                                    <div key=${i} style="display: flex; gap: 4px; margin-bottom: 4px; align-items: center;">
                                        ...
                                        <button class="btn btn-ghost" style="padding: 2px; min-width: 0;" onClick=${() => removeHeaderKV(i)}>
                                            <${Icon} name="x" size=${10} />
                                        </button>
                                    </div>
                                `)}
                ...
                <div style="flex: 1; display: flex; flex-direction: column; min-width: 0;">
                    ...
                    <div style="flex: 1; overflow: auto; background: var(--bg-primary); border: 1px solid var(--border-weak); border-radius: var(--radius-md); padding: 12px;">
                        ...
                        ${activePanel === 'response' && response && html`
                            <pre class="text-mono" style="font-size: 12px; line-height: 1.5; color: var(--text-primary); white-space: pre-wrap; word-break: break-word; margin: 0;">${response.error
    ? response.error
    : JSON.stringify(response.response || response, null, 2)}</pre>
                        `}
                        ${activePanel === 'preview' && previewData && html`
                            <div style="font-size: 12px;">
                                <div style="margin-bottom: 12px;">
                                    <span class="badge" style="margin-right: 8px;">${previewData.method}</span>
                                    <span class="text-mono" style="color: var(--color-info);">${previewData.url}</span>
            ...
            <${Modal} isOpen=${showSaveModal} title=${editRequestId ? 'Update Request' : 'Save Request'} onClose=${() => setShowSaveModal(false)}>
                <div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="flex gap-2">
                            <select class="form-select" style="flex: 1;" value=${saveFolderName}
                                onChange=${(e) => setSaveFolderName(e.target.value)}>
                                <option value="">— No Folder —</option>
                                ${folderNames.map(f => html`<option key=${f} value=${f}>${f}</option>`)}
                            </select>
                            <input class="form-input" style="flex: 1;" value=${saveFolderName}
                                onInput=${(e) => setSaveFolderName(e.target.value)}
            ...
            <${ConfirmDialog}
                ...
                title="Delete Folder"
                message=${'Delete folder "' + (deleteFolderTarget || '') + '" and all its requests?'}
